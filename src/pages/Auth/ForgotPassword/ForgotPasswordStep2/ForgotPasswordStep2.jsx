import React, { useState } from "react";
import { useNavigate } from 'react-router-dom';
import Logo from '../../../../assets/light/LogoRightText.png'
import ForgotPassCSS from '../ForgotPassword.module.css'
import { Image } from 'primereact/image';
import { InputText } from "primereact/inputtext";
import { Button } from 'primereact/button';

function ForgotPasswordStep2() {
    const navigate = useNavigate();

    const irALogin = () => {
      navigate('/login'); 
    };

    const forgotPasswordStep3 = () => {
      navigate('/recuperarContraseniaPaso3'); 
    };
  
    const [inputEmail, setInputEmail] = useState('ejemplo@ejemplo.com');
    const [inputCode, setInputCode] = useState('');

    return (
      <div className={`flex justify-content-center align-items-center ${ForgotPassCSS.content}`}>
        <div className={`
                flex 
                justify-content-center 
                align-items-center 
                flex-column 
                p-5
                border-round-xl
                ${ForgotPassCSS.fondoLogin}
                `
            }>
            <Image 
                src={Logo} 
                alt="Image" 
                width="250" 
            />
            <p className="p-0 m-0 mb-1 mt-4 text-xl">Recuperar Contraseña</p>
        
            <form className="lg:w-25rem">
                <div>
                  <p className={`${ForgotPassCSS.textWarning} w-20rem`}>
                    <i className={`pi pi-exclamation-triangle ${ForgotPassCSS.textWarning}`}></i>
                    <span> </span>
                    Por favor revise el Correo Electrónico declarado en la app de Trigo Limpio (frullo@trigolimpio.com) y luego ingrese el código enviado
                  </p>
                </div>
                <div className="flex flex-column gap-2 w-full mb-4">
                        <label htmlFor="email">Email</label>
                        <InputText 
                          value={inputEmail}
                          onChange={(e) => setInputEmail(e.target.value)}
                          type="email" 
                          id="email" 
                          aria-describedby="email-help" 
                          className="
                            bg-transparent 
                            text-white
                          " 
                          readOnly
                        />
                        <small id="username-help" style={{ color: "var(--red_300)" }} className="hidden">
                            !!!!!!!!!!!!!!AGREGAR VALIDACIONNN!!!!!!!!!!!!!!!
                        </small>
                </div>
                <div className="flex flex-column gap-2 w-full mb-4">
                        <label htmlFor="code">Codigo</label>
                        <InputText 
                          value={inputCode}
                          onChange={(e) => setInputCode(e.target.value)}
                          id="code" 
                          aria-describedby="code-help" 
                          className="
                            bg-transparent 
                            text-white
                          " 
                        />
                        <small id="username-help" style={{ color: "var(--red_300)" }} className="hidden">
                            !!!!!!!!!!!!!!AGREGAR VALIDACIONNN!!!!!!!!!!!!!!!
                        </small>
                </div>
                <div className="flex gap-3 sm:gap-5 flex-wrap sm:flex-nowrap">
                    <Button 
                      type="submit" 
                      label="Validar Codigo" 
                      className={`
                        w-full  
                        border-none 
                        text-white 
                        ${ForgotPassCSS.btnForgot}
                      `}
                      onClick={() => forgotPasswordStep3()}
                    />
                    <Button 
                      type="button" 
                      label="Cancelar" 
                      className={`
                        w-full  
                        border-none 
                        text-white 
                        ${ForgotPassCSS.btnForgot}
                      `} 
                      onClick={() => irALogin()}
                    />
                </div>
            </form>
        </div>
      </div>
    )
}

export default ForgotPasswordStep2
